  ...
  if (validating) {
    return (
      <div className="auth-glass-card py-5 px-6 sm:py-7 sm:px-12 max-w-130 mx-auto w-full animate-slide-up flex flex-col items-center gap-5 text-center min-h-55 justify-center">
        ...
        <p className="text-muted-foreground text-sm">{t("reset.verifying")}</p>
      </div>
    );
  }
  ...
  if (validationError) {
    return (
      <div className="auth-glass-card py-5 px-6 sm:py-7 sm:px-12 max-w-130 mx-auto w-full animate-slide-up flex flex-col items-center gap-4 text-center">
        <ShieldX className="w-14 h-14 text-error" />
        <h1 className="text-2xl font-extrabold tracking-[-0.02em] text-gradient-primary">{t("reset.invalidLinkTitle")}</h1>
  ...
  if (resetDone) {
    return (
      <div className="auth-glass-card py-5 px-6 sm:py-7 sm:px-12 max-w-130 mx-auto w-full animate-slide-up flex flex-col items-center gap-4 text-center">
        ...
        <h1 className="text-2xl font-extrabold tracking-[-0.02em] text-gradient-primary">{t("reset.successTitle")}</h1>
    ...
    <div className="auth-glass-card py-5 px-6 sm:py-7 sm:px-12 max-w-130 mx-auto w-full animate-slide-up">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        <div className="flex flex-col gap-1.5">
          <label htmlFor="newPassword" className="text-[13px] text-muted-foreground tracking-wide pl-1">
          ...
          <div className="relative group">
            ...
            <input
              ...
              className={`input-base w-full rounded-lg py-3 pl-12 pr-4 text-on-surface text-sm font-medium border border-outline-variant bg-surface-container-lowest focus:border-primary focus:ring-2 focus:ring-primary/20 [&::-ms-reveal]:hidden [&::-ms-clear]:hidden ${
                apiError?.fieldMessage("newPassword") ? "border-error focus:border-error focus:ring-error/20" : ""
              }`}
            />
          </div>
          {apiError?.fieldMessage("newPassword") && (
            <div className="flex items-center gap-1.5 pl-1 animate-error-slide">
              ...
              <p className="text-xs text-error">{apiError.fieldMessage("newPassword")}</p>
            </div>
          )}
        </div>

        <div className="flex flex-col gap-1.5">
          <label htmlFor="confirmPassword" className="text-[13px] text-muted-foreground tracking-wide pl-1">
          ...
          <div className="relative group">
            ...
            <input
              ...
              className={`input-base w-full rounded-lg py-3 pl-12 pr-4 text-on-surface text-sm font-medium border border-outline-variant bg-surface-container-lowest focus:border-primary focus:ring-2 focus:ring-primary/20 [&::-ms-reveal]:hidden [&::-ms-clear]:hidden ${
                apiError?.fieldMessage("confirmPassword") ? "border-error focus:border-error focus:ring-error/20" : ""
              }`}
            />
          </div>
          {apiError?.fieldMessage("confirmPassword") && (
            <div className="flex items-center gap-1.5 pl-1 animate-error-slide">
              <AlertCircle className="w-3.5 h-3.5 text-error shrink-0" />
              <p className="text-xs text-error">{apiError.fieldMessage("confirmPassword")}</p>
            </div>
          )}
        </div>
        ...
        {formError && (!apiError || apiError.fields.length === 0) && (
          <div className="flex items-center justify-center gap-1.5 -mt-1 animate-error-slide">
            <AlertCircle className="w-3.5 h-3.5 text-error shrink-0" />
            <p className="text-xs text-error">{formError.message}</p>
          </div>
        )}
