...
import {
  getMediaBackedClipMaxTimelineEnd,
  toMs,
  toSeconds,
} from "../../utils/time";
...
import { extractAudioFromVideo } from "../../engine/audioExtraction";
import { fileMap } from "./projectSlice.state";
...
export const createProjectTimelineActions: StateCreator<
  EditorStore,
  [],
  [],
  ProjectTimelineActions
> = (set, get) => ({
  addClip(clip) {
    ...
    const prepared: Clip = (() => {
      if (clip.type === "video" || clip.type === "audio") {
        const clampedSpeed = Math.max(
          MIN_SPEED,
          Math.min(MAX_SPEED, clip.speed ?? DEFAULT_SPEED),
        );
        return {
          ...clip,
          speed: clampedSpeed,
    ...
    set((state) => ({
      ...
      project: {
        ...
        clipGroups: (state.project.clipGroups ?? [])
          .map((group) => ({
            ...group,
            memberClipIds: group.memberClipIds.filter(
              (id) =>
                !state.project.tracks
                  .find((t) => t.id === trackId)
                  ?.clips.some((c) => c.id === id),
  ...
  reorderTrack(sourceTrackId, targetTrackId) {
    ...
    set((state) => {
      ...
      return {
        project: {
          ...state.project,
          tracks: reordered.sort((a, b) => a.order - b.order),
        },
      };
    });
  },
  ...
  resizeClip(clipId, newEnd) {
    ...
    set((state) => {
      ...
      const nextTracks = state.project.tracks.map((track) => {
        ...
        const maxEnd =
          getMediaBackedClipMaxTimelineEnd(target) ?? Number.POSITIVE_INFINITY;
        const clampedEnd = Math.max(minEnd, Math.min(newEnd, maxEnd));
        const nextTrack = applyTrackRippleForEndChange(
          track,
          clipId,
          clampedEnd,
        );
  ...
  async extractAudioFromClip(clipId) {
    const state = get();
    const sourceClip = findClipById(state.project.tracks, clipId);
    if (!sourceClip || sourceClip.type !== "video") return null;

    const sourceFile = fileMap.get(sourceClip.mediaId);
    if (!sourceFile) {
      console.warn("Cannot extract audio: source video file is unavailable");
      return null;
    }

    const existingAudioTrack = state.project.tracks.find(
      (t) => t.type === "audio",
    );

    // Check for collision before extracting
    const hasOverlap =
      existingAudioTrack?.clips.some(
        (clip) =>
    ...
    if (hasOverlap) {
      ...
      console.warn(
        "Cannot extract audio: overlapping clip exists on audio track",
      );
      return null;
    }

    try {
      const extractedFile = await extractAudioFromVideo(
        sourceClip.mediaId,
        sourceFile,
        sourceClip.mediaStart,
        sourceClip.mediaEnd,
      );
      const extractedMedia = await get().addMedia(extractedFile);
      const targetTrack = existingAudioTrack ?? get().addTrack("audio");

      const newClip: Clip = {
        id: generateId(),
        type: "audio",
        mediaId: extractedMedia.id,
      ...
      console.error("Cannot extract audio:", error);
      return null;
    }
  },
  ...
  splitClip(clipId, time) {
    set((state) => {
      ...
      const firstHalf: Clip = {
        ...
        ...("mediaEnd" in clip
          ? { mediaEnd: toSeconds(toMs(clip.mediaStart + splitPoint)) }
          : {}),
      } as Clip;
      ...
      const secondHalf: Clip = {
        ...
        ...("mediaStart" in clip
          ? { mediaStart: toSeconds(toMs(clip.mediaStart + splitPoint)) }
          : {}),
      } as Clip;
